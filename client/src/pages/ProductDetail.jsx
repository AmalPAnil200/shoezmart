  ...
  if (error || !product)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/" className="text-orange-500 font-bold underline">
          Back to Shop
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      <div className="max-w-7xl mx-auto px-6 pt-10">
        <Link
          to="/"
          className="text-zinc-400 hover:text-zinc-900 text-xs font-black flex items-center gap-2 mb-10 transition-colors tracking-widest uppercase"
        >
          ← BACK TO COLLECTION
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-8">
              <span className="text-4xl font-bold text-zinc-900 italic">
                ₹{product.price}
              </span>
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${product.stock > 0 ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
              >
                {product.stock > 0
            ...
            <div className="mb-10">
              <div className="flex justify-between items-center mb-4">
                ...
                <button className="text-[10px] font-bold text-zinc-400 underline underline-offset-4 uppercase">
                  Size Guide
                </button>
              </div>
              <div className="grid grid-cols-5 gap-3">
                {sizes.map((size) => (
                  <button
                    ...
                    className={`py-4 rounded-2xl border-2 font-black transition-all active:scale-90 text-sm ${
                      selectedSize === size
            ...
            <p className="text-zinc-500 text-lg leading-relaxed mb-10 border-l-4 border-zinc-100 pl-6 italic">
              {product.description ||
                "Designed for ultimate performance and street-ready style, this silhouette provides unmatched comfort and durability."}
            </p>
